...
#            path2.append('start')
                ...
                if math.exp(math.log(d['<LITERAL>']['pLitToLit'])+math.log(score0[j-1])+lambdaNB0*math.log(NBlabels[index][0])) >= math.exp(lambda10*math.log(d['<METAPHOR>']['pMetToLit'])+math.log(score1[j-1])+lambdaNB0*math.log(NBlabels[index][0])): 
                    score0.append(math.exp(math.log(d['<LITERAL>']['pLitToLit'])+math.log(score0[j-1])+lambdaNB0*math.log(NBlabels[index][0])))
                    path0.append('0')
                else: 
                    score0.append(math.exp(math.log(d['<METAPHOR>']['pMetToLit'])+math.log(score1[j-1])+lambdaNB0*math.log(NBlabels[index][0])))
                    path0.append('1')
                if math.exp(math.log(d['<METAPHOR>']['pMetToMet'])+math.log(score1[j-1])+lambdaNB1*math.log(NBlabels[index][1])) >= math.exp(math.log(d['<LITERAL>']['pLitToMet'])+math.log(score0[j-1])+lambdaNB1*math.log(NBlabels[index][1])):
                    score1.append(math.exp(math.log(d['<METAPHOR>']['pMetToMet'])+math.log(score1[j-1])+lambdaNB1*math.log(NBlabels[index][1])))
                    path1.append('1')
                else: 
                    score1.append(math.exp(math.log(d['<LITERAL>']['pLitToMet'])+math.log(score0[j-1])+lambdaNB1*math.log(NBlabels[index][1])))
                    path1.append('0')
        index += 1
