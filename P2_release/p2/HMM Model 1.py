...
for i in range(len(tsentences)): 
    vlabel_list = tlabels[i].strip('][').split(', ')
...
#            path2.append('start')
                ...
                if (lambdaLL*math.exp(math.log(d['<LITERAL>']['pLitToLit'])
                +math.log(score0[j-1])+math.log(d[word]['litLP'])) >= 
                math.exp(math.log(d['<METAPHOR>']['pMetToLit'])
                +math.log(score1[j-1])+math.log(d[word]['litLP']))): 
                    
                    score0.append(lambdaLL*math.exp(math.log(d['<LITERAL>']['pLitToLit'])
                    +math.log(score0[j-1])+math.log(d[word]['litLP'])))
                    
                    path0.append('0')
                else: 
                    score0.append(math.exp(math.log(d['<METAPHOR>']['pMetToLit'])
                    +math.log(score1[j-1])+math.log(d[word]['litLP'])))
                    
                    path0.append('1')
                if (math.exp(math.log(d['<METAPHOR>']['pMetToMet'])+math.log(score1[j-1])
                +math.log(d[word]['metLP'])) >= math.exp(math.log(d['<LITERAL>']['pLitToMet'])
                +math.log(score0[j-1])+math.log(d[word]['metLP']))):
                    
                    score1.append(math.exp(math.log(d['<METAPHOR>']['pMetToMet'])
                    +math.log(score1[j-1])+math.log(d[word]['metLP'])))
                    
                    path1.append('1')
                else: 
                    score1.append(math.exp(math.log(d['<LITERAL>']['pLitToMet'])
                    +math.log(score0[j-1])+math.log(d[word]['metLP'])))
